...
```ts
import {LoadOptions} from "https://deno.land/x/tsa@v0.0.38/mod.ts"
```
...
// curl 'https://raw.githubusercontent.com/jeremiah-shaulov/tsa/v0.0.38/generated-doc/type.LoadOptions/README.md' | perl -ne '$y=$1 if /^```(.)?/;  print $_ if $y&&$m;  $m=$y&&($m||m~<example-7jcr>~)' > /tmp/example-7jcr.ts
// deno run --allow-env --allow-net --allow-read --allow-write /tmp/example-7jcr.ts

import {tsa, defaultLoad, printDiagnostics} from 'https://deno.land/x/tsa@v0.0.38/mod.ts';
...
```ts
import {tsa, defaultResolve, defaultLoad, printDiagnostics} from 'https://deno.land/x/tsa@v0.0.38/mod.ts';
